...
def perceptual(pred, gt, mask=None, with_grad=False, use_mask=False, no_head=False, device='cpu'):
    """
    ...
    def _run(pred, gt, mask):
        ...
        errors = perc_loss_net(pred.to(device), gt.to(device), normalize=True)
        return errors
...
def run(output_dir, gt_dir, pred_file_name, load_npz=False, no_cloth=False, no_head=False):
    if load_npz:
        ...
        for subfolder_i in range(len(subfolders)):
            ...
            cam_file = '{0}/flame_params_reenact.json'.format(instance_dir)
            ...
            files = sorted(os.listdir(os.path.join(output_dir, subfolder, pred_file_name)), key=lambda x: int(Path(x).stem))
